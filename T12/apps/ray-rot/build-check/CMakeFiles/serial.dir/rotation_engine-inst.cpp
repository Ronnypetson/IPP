...
*/
bool RotateEngine::init(Image *input, unsigned int angle, string destname) {

  __instrument_this_function();
...
*   Function: run
*   -------------
*   Runs the benchmark kernel. When completed successfully, done will be set to
* true
...
void RotateEngine::run() {
  ...
  /* Steps for rotation:
          ...
          2. For each pixel in target image, do
                  ...
                  - for origin location, sample and filter 4 closest neighbour
     pixels
  ...
  */
  ...
  output->createImageFromTemplate(target_w, target_h, RGB_DEPTH);
  ...
  for (int i = 0; i < target_h; i++) {
    for (int j = 0; j < target_w; j++) {
      ...
      if (input->containsPixel(&origin_pix)) {
        ...
        for (int k = 0; k < 4; k++) {
          samples[k][0] = (int)(origin_pix.x + x_offset_source) +
                          ((k == 2 || k == 3) ? 1 : 0);
          samples[k][1] = (int)abs(origin_pix.y - y_offset_source) +
                          ((k == 1 || k == 3) ? 1 : 0);
...
void RotateEngine::finish() {
  ...
  if (!writeOutImage()) {
    fprintf(stderr, "Could Not Write Rotation Output\n");
  }
  output->clean();
  delete output;
}
...
void RotateEngine::printRotationState() {
  ...
  fprintf(stdout, "Rotation Kernel:\n");
  fprintf(stdout, "Width: %d\t Height: %d\n", input->getWidth(),
          input->getHeight());
  fprintf(stdout, "Pixels: %.2fM\t Angle: %d°\n",
          (double)(input->getWidth() * input->getHeight()) / 1000000.0,
          (int)angle);
  fprintf(stdout, "Dest. File: %s\n\n", destname.c_str());
}
...
*/
inline void RotateEngine::rotatePoint(float *pt, float *target,
                                      unsigned int angle) {
  ...
  ((Coord *)target)->x =
      ((Coord *)pt)->x * cos(rad) - ((Coord *)pt)->y * sin(rad);
  ((Coord *)target)->y =
      ((Coord *)pt)->x * sin(rad) + ((Coord *)pt)->y * cos(rad);
}
...
double RotateEngine::round(double num, int digits) {
  ...
  if (digits > (sizeof(v) / sizeof(double))) {
    return num;
  }
...
*/
inline void RotateEngine::filter(Pixel *colors, Pixel *dest,
                                 Coord *sample_pos) {
...
*	---------------------------
*	Linearly interpolates two pixel colors according to a given weight
*factor.
*/
inline void RotateEngine::interpolateLinear(Pixel *a, Pixel *b, Pixel *dest,
                                            float weight) {
