 ...
 *  from disk into memory, fills jitter tables and sets global raytracing
 * params.
 */
bool RayEngine::init(string srcname, int xres, int yres, int rpp) {
  int i;
  for (i = 0; i < NRAN; i++) {
  ...
  cout << "Loading from file " << srcname << endl;
  ...
  out.createImageFromTemplate(xres, yres, RGB_DEPTH);
  return true;
};
...
void RayEngine::run() {
  for (int i = 0; i < yres; i++) {
    render_scanline(xres, yres, i, out, rays_per_pixel);
  }
}
 ...
 */
Image *RayEngine::getOutputImage() { return &out; }
...
void RayEngine::render_scanline(int xsz, int ysz, int sl, Image &out,
                                int samples) {
  int i, s;
 ...
 * hit.
 * Also the surface point parameters like position, normal, etc are returned
 * through
 * the sp pointer if it is not NULL.
 */
int RayEngine::ray_sphere(const struct sphere *sph, struct ray ray,
                          struct spoint *sp) {
